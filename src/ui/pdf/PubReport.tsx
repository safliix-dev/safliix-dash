...
const styles = StyleSheet.create({
  ...
  colTitle:  { width: "50%", paddingHorizontal: 6 },
  colViews:  { width: "20%", paddingHorizontal: 6, textAlign: "center" },
  colClicks: { width: "20%", paddingHorizontal: 6, textAlign: "center" },
...
export function PubReport({ clientName, periodStart, periodEnd, entries }: Props) {
  const rows = [...entries];
  while (rows.length < 10) {
    rows.push({ order: "", title: "", views: "", clicks: "" });
  }
    ...
    <Document>
      <Page size="A4" orientation="landscape" style={styles.page}>
        <View style={styles.banner}>
          <Text style={styles.bannerText}>RAPPORT DES PUBS PAR CLIENT</Text>
        </View>

        <View style={styles.infoRow}>
          <Text style={styles.infoCell}>
            <Text style={styles.infoCellLabel}>CLIENT :</Text> {clientName}
          </Text>
