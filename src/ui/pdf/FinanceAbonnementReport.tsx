...
export type FinanceAbonnementEntry = {
  order: string;
...
type Props = {
  periodStart: string;
  periodEnd: string;
  entries: FinanceAbonnementEntry[];
};
...
const styles = StyleSheet.create({
  ...
  colOrder:        { width: "6%",  paddingHorizontal: 3 },
  colRightsholder: { width: "12%", paddingHorizontal: 3 },
  colTitle:        { width: "15%", paddingHorizontal: 3 },
  colCategory:     { width: "9%",  paddingHorizontal: 3 },
  colFormat:       { width: "7%",  paddingHorizontal: 3 },
  colSubs:         { width: "9%",  paddingHorizontal: 3, textAlign: "center" },
  colCatalog:      { width: "11%", paddingHorizontal: 3, textAlign: "center" },
  colViewing:      { width: "11%", paddingHorizontal: 3, textAlign: "center" },
  colPercent:      { width: "10%", paddingHorizontal: 3, textAlign: "center" },
  colRevenue:      { width: "10%", paddingHorizontal: 3, textAlign: "center" },
  headerText: { fontSize: 7, fontWeight: 700 },
  rowText: { fontSize: 8 },
...
export function FinanceAbonnementReport({ periodStart, periodEnd, entries }: Props) {
  const rows = [...entries];
  while (rows.length < 10) {
    rows.push({ order: "", rightsholderName: "", title: "", category: "", format: "", subscriptions: "", catalogDuration: "", viewingTime: "", viewingPercentage: "", revenue: "" });
  }
    ...
    <Document>
      <Page size="A4" orientation="landscape" style={styles.page}>
        <View style={styles.banner}>
          <Text style={styles.bannerText}>RAPPORT GENERAL FINANCIER DES ABONNEMENTS DE FILMS</Text>
        </View>
